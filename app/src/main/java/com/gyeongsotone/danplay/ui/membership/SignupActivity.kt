...
class SignupActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (signupIdState == "wrong_format") {
            binding.idFail.text = "단국대 이메일을 입력해주세요."
    ...
    override fun onClick(v: View?) {
        for (i in 0 until preferButton.size step (1)) {
            if (preferButton[i].isChecked) {
                preferButtonState[i] = 1
                preferButton[i].background = ContextCompat.getDrawable(
                    this,
                    R.drawable.btn_event_stroke
                )
            } else {
                preferButtonState[i] = 0
    ...
    private fun signup() {
        ...
        val userPwd = binding.passwordEdittext.text.toString()
        val pwdCheck = binding.passwordCheckEdittext.text.toString()
        val userName = binding.usernameEdittext.text.toString()
        val userBirth = binding.birthEdittext.text.toString()
        val userInfo = arrayOf(userName, userBirth)
        // 입력 칸이 비어있다면 리턴
        if (TextUtils.isEmpty(userPwd)) {
            wrongInput = 1
            binding.signUpPasswordFail.text = "비밀번호를 입력해주세요."
            binding.signUpPasswordFail.visibility = View.VISIBLE
            binding.signupPasswordCheckFail.text = "비밀번호를 입력해주세요."
        ...
        if ((preferButtonState[0] or preferButtonState[1] or preferButtonState[2]
                    or preferButtonState[3] or preferButtonState[4]) == 0
        ) {
            wrongInput = 1
        ...
        if (userEmail.contains("@")) {
            if (((emailArr[0].length == 8) and (emailArr[1].equals("dankook.ac.kr")))) {
                ...
                wrongInput = 1
                binding.idFail.text = "올바른 양식의 아이디를 입력해주세요."
                binding.idFail.visibility = View.VISIBLE
            }
        } else {
            wrongInput = 1
